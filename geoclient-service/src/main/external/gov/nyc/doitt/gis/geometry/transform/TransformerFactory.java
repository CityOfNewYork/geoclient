...
public class TransformerFactory {
	...
	public Transformer getInstance(String sourceCrs, String targetCrs) throws CrsConfigurationException {
		return new GeoToolsTransformer(
				getCoordinateReferenceSystem(sourceCrs),
				getCoordinateReferenceSystem(targetCrs));
	}	
	
	public CoordinateReferenceSystem getCoordinateReferenceSystem(
			String epsgNumber) throws CrsConfigurationException {
		try {
			return CRS.decode(epsgNumber);
		} catch (FactoryException e) {
			throw new CrsConfigurationException("Error decoding EPSG number "
					+ epsgNumber, e);
		}
	}
}
