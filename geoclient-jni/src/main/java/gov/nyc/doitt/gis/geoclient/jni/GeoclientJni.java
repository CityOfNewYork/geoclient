...
public class GeoclientJni implements Geoclient{
    ...
    public void callgeo(ByteBuffer work_area1, ByteBuffer work_area2)  {
        ...
        callgeo1(
            work_area1_is_direct ? work_area1 : work_area1.array(),
            work_area1_is_direct ? getDirectBufferByteOffset(work_area1) : getIndirectBufferByteOffset(work_area1),
            work_area1_is_direct,
            work_area2_is_direct ? work_area2 : work_area2.array(),
            work_area2_is_direct ? getDirectBufferByteOffset(work_area2) : getIndirectBufferByteOffset(work_area2),
     ...
     *
     * @param work_area1 direct or array-backed {@link java.nio.ByteBuffer} or a {@link java.lang.byte[]}
     * @param work_area1_byte_offset array offset
     * @param work_area1_is_direct whether the work_area1 arugment is a direct {@link java.nio.ByteBuffer} 
     * @param work_area2 direct or array-backed {@link java.nio.ByteBuffer} or a {@link java.lang.byte[]}
     * @param work_area2_byte_offset array offset
     * @param work_area2_is_direct whether the work_area2 arugment is a direct {@link java.nio.ByteBuffer} 
     *
     */
     ...
     *
     */
    public void callgeo(byte[] work_area1,
                        int work_area1_offset,
                        byte[] work_area2,
    ...
    private void verifyOffset(WorkArea workArea, byte[] buffer, int offset) {
        if(buffer != null && buffer.length <= offset) {
            throw new RuntimeException(String.format("%s byte[] argument offset %d equals or exceeds array length %d", workArea, offset, buffer.length));
        }
    }

    private void logBufferType(WorkArea workArea, BufferType bufferType, boolean isNull) {
        if(isNull) {
            System.out.println(String.format("%s buffer type: %s, however it is null ", workArea, bufferType));
        } else {
