...
public abstract class AbstractRegexParser implements Parser
{
	...
	{
		LOGGER.info("{} could not match tokens in chunk '{}' using pattern {}",getName(),parseContext.currentChunkText(),pattern);
	}
	
	protected Token newToken(TokenType type, MatchResult matchResult, int groupNumber)
	{
		Token token = new Token(type,matchResult.group(groupNumber),matchResult.start(groupNumber),matchResult.end(groupNumber));
	...
	protected void handleMatch(Match match, ChunkType chunkType)
	{
		...
		for (RegexTokenGroup matchGroup : matchGroups)
		{
			...
			if(matchResult.group(groupNumber)!=null)
			{
				...
				LOGGER.info("{} found tokens in chunk '{}' using pattern {}",getName(),currentChunk.getText(),matchGroup.getPattern());
